...
#==========================Diffusion_reaction 1-1D===========================
def Diffusion_reaction1_1D_sol(X,eva_torch,save_path_data):
    expr_str = "e**(−2*t)*sin(x1)"
    y = torch.exp(-2*X[1])*torch.sin(X[0])
...
def Diffusion_reaction1_1D_eva(func, X):
    #=============du2_dx1================
    try:
    ...
    pde_residual = du1_dt - 3*du2_dx1 -func
    mse1 = F.mse_loss(pde_residual , torch.zeros_like(pde_residual))
    return mse1


#==============================Diffusion_reaction 1-2D===========================
def Diffusion_reaction1_2D_sol(X,eva_torch,save_path_data):
    expr_str = "e**(−2*t)*sin(x1)+e**(−2*t)*sin(x2)"
    y = torch.exp(-2*X[2])*torch.sin(X[0])+torch.exp(-2*X[2])*torch.sin(X[1])
...
def Diffusion_reaction1_2D_eva(func, X):
    #=============du2_dx1================
    try:
        du2_dx1 = torch.autograd.grad(func.sum(),X[0], create_graph=True, retain_graph=True)
        if du2_dx1[0] is None:
    ...
    pde_residual = du1_dt - 3*(du2_dx1 + du2_dx2) -func
    mse1 = F.mse_loss(pde_residual , torch.zeros_like(pde_residual))
    return mse1


#==============================Diffusion_reaction 1-3D===========================
def Diffusion_reaction1_3D_sol(X,eva_torch,save_path_data):
    expr_str = "e**(−2*t)*sin(x1)+e**(−2*t)*sin(x2)+e**(−2*t)*sin(x3)"
    y = torch.exp(-2*X[3])*torch.sin(X[0])+torch.exp(-2*X[3])*torch.sin(X[1])+torch.exp(-2*X[3])*torch.sin(X[2])
...
def Diffusion_reaction1_3D_eva(func, X):
    #=============du2_dx1================
    try:
        du2_dx1 = torch.autograd.grad(func.sum(),X[0], create_graph=True, retain_graph=True)
        if du2_dx1[0] is None:
    ...
    pde_residual = du1_dt - 3*(du2_dx1 + du2_dx2 + du2_dx3) -func
    mse1 = F.mse_loss(pde_residual , torch.zeros_like(pde_residual))
    return mse1
...
#==========================Diffusion_reaction 2-1D===========================
def Diffusion_reaction2_1D_sol(X,eva_torch,save_path_data):
    expr_str = "e**(−3*t)*sin(x1)"
    y = torch.exp(-3*X[1])*torch.sin(X[0])
...
def Diffusion_reaction2_1D_eva(func, X):
    #=============du2_dx1================
    try:
        du2_dx1 = torch.autograd.grad(func.sum(),X[0], create_graph=True, retain_graph=True)
        if du2_dx1[0] is None:
    ...
    pde_residual = du1_dt - 2*du2_dx1 +func
    mse1 = F.mse_loss(pde_residual , torch.zeros_like(pde_residual))
    return mse1


#==============================Diffusion_reaction 2-2D===========================
def Diffusion_reaction2_2D_sol(X,eva_torch,save_path_data):
    expr_str = "e**(−3*t)*sin(x1)+e**(−3*t)*sin(x2)"
    y = torch.exp(-3*X[2])*torch.sin(X[0])+torch.exp(-3*X[2])*torch.sin(X[1])
...
def Diffusion_reaction2_2D_eva(func, X):
    #=============du2_dx1================
    try:
        du2_dx1 = torch.autograd.grad(func.sum(),X[0], create_graph=True, retain_graph=True)
        if du2_dx1[0] is None:
    ...
    try:
        ...
        print("Error in computing grad:", error)
        du1_dt = torch.full(X[2].size(), 100, dtype=X[2].dtype)
    pde_residual = du1_dt - 2*(du2_dx1 + du2_dx2) + func
    mse1 = F.mse_loss(pde_residual , torch.zeros_like(pde_residual))
    return mse1


#==============================Diffusion_reaction 2-3D===========================
def Diffusion_reaction2_3D_sol(X,eva_torch,save_path_data):
    expr_str = "e**(−3*t)*sin(x1)+e**(−3*t)*sin(x2)+e**(−3*t)*sin(x3)"
    y = torch.exp(-3*X[3])*torch.sin(X[0])+torch.exp(-3*X[3])*torch.sin(X[1])+torch.exp(-3*X[3])*torch.sin(X[2])
...
def Diffusion_reaction2_3D_eva(func, X):
    #=============du2_dx1================
    try:
        du2_dx1 = torch.autograd.grad(func.sum(),X[0], create_graph=True, retain_graph=True)
        if du2_dx1[0] is None:
    ...
    pde_residual = du1_dt - 2*(du2_dx1 + du2_dx2 + du2_dx3) + func
    mse1 = F.mse_loss(pde_residual , torch.zeros_like(pde_residual))
    return mse1
