...
def ExecuteProgram (input_var_data, program_tokens, free_const_values=None):
    ...
    start = n_tokens - 1
    const_step = len(free_const_values)-1
...
def torch_tensor_fft():
    x_tensor = torch.tensor([1, 2, 3, 4, 5, 6, 7, 8], dtype=torch.float32)

    # transform 1D tensor to fft
    # https://pytorch.org/docs/stable/generated/torch.fft.html




def ComputeInfixNotation (program_tokens,free_const_values=None):
    """
    ...
    for i in range (start, -1, -1):
        ...
        if token.arity == 0:
            ...
            if token.var_type == 2 and free_const_values is not None:
                res = str(free_const_values[const_step])
                const_step = const_step-1
        elif token.arity == 1:
...
def BatchFreeConstOpti (progs, X, y_target, free_const_opti_args, mask = None, n_cpus = 1, parallel_mode = False):
    ...
    # Non parallel mode
        ...
        for i in pb(range(progs.batch_size)):
            ...
            if mask[i] and progs.n_free_const_occurrences[i]:
                # Getting minimum executable skeleton pickable program
                free_const_temp = torch.tensor(np.ones(progs.n_free_const_occurrences[i]))
                prog = progs.get_prog(i, skeleton=True)
                prog.free_const_values = free_const_temp
                task_free_const_opti(prog, X = X, y_target = y_target, free_const_opti_args = free_const_opti_args)
                # task_free_const_opti(prog, X=X, y_target=y_target,
                #                      free_const_opti_args=free_const_opti_args)
                # y_pred, X_temp = prog.torch_exec(X, prog.tokens, prog.free_const_values)
                # result = eva.torch_Poisson1d(y_pred, X_temp,y_target)
                # results.append(result)
                progs.free_consts.values[i]=prog.free_const_values
